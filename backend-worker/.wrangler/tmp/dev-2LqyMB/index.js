...
__name(splitIntoBullets, "splitIntoBullets");
router.get("/entries", async (c) => {
  try {
    ...
    const { data: entries, error } = await supabase.from("journal_entries").select("*").order(orderColumn, { ascending }).range(offset, offset + limit - 1);
    if (error) throw error;
    const { count, error: countError } = await supabase.from("journal_entries").select("*", { count: "exact", head: true });
...
router.post("/entry", async (c) => {
  try {
    ...
    const { data: existingStats, error: statsFetchError } = await supabase.from("progress_stats").select("*").eq("date", today).maybeSingle();
    if (statsFetchError) {
      console.warn("Error fetching progress stats:", statsFetchError.message);
    }
    if (existingStats) {
      const { error: updateError } = await supabase.from("progress_stats").update({
      ...
      const { error: statsInsertError } = await supabase.from("progress_stats").insert({
        date: today,
...
router.put("/entry/:id", async (c) => {
  try {
    ...
    const { data: existing, error: fetchError } = await supabase.from("journal_entries").select("*").eq("id", id).single();
    if (fetchError) {
    ...
    const { data: updated, error } = await supabase.from("journal_entries").update(updateData).eq("id", id).select().single();
    if (error) throw error;
    return c.json({
      success: true,
      data: {
...
router.get("/search", async (c) => {
  try {
    ...
    const { data: results, error } = await query.order("created_at", { ascending: false }).limit(50);
    if (error) throw error;
    return c.json({
      success: true,
      data: results || [],
...
// services/gemini-translation.js
async function translateWithGeminiApi(text, targetLang = "German", env = null) {
  const apiKey = env?.GEMINI_API_KEY || globalThis.GEMINI_API_KEY || (typeof process !== "undefined" ? process.env?.GEMINI_API_KEY : void 0);
  if (!apiKey) {
...
__name(translateWithGeminiApi, "translateWithGeminiApi");
async function translateWithGemini(text, env = null) {
  return translateWithGeminiApi(text, "German", env);
}
__name(translateWithGemini, "translateWithGemini");
async function translateToEnglish(text, env = null) {
  return translateWithGeminiApi(text, "English", env);
}
__name(translateToEnglish, "translateToEnglish");
...
async function translateToGerman(text, env = null) {
  try {
...
__name(translateToGerman, "translateToGerman");
async function translateToEnglishWithFallback(text, env = null) {
  try {
...
__name(translateToEnglishWithFallback, "translateToEnglishWithFallback");
async function translateMultipleSentences(text, env = null) {
  if (!text || typeof text !== "string") {
    throw new Error("Invalid text provided for translation");
  }
...
__name(translateMultipleSentences, "translateMultipleSentences");

// services/vocabulary-extractor.js
async function getVocabularyStats(env) {
  const supabase = getSupabaseClient(env);
  const { count: total, error: totalError } = await supabase.from("vocabulary").select("*", { count: "exact", head: true });
  if (totalError) throw totalError;
  const sevenDaysAgo = /* @__PURE__ */ new Date();
  sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
  const { count: thisWeek, error: weekError } = await supabase.from("vocabulary").select("*", { count: "exact", head: true }).gte("first_seen", sevenDaysAgo.toISOString());
  if (weekError) throw weekError;
  const thirtyDaysAgo = /* @__PURE__ */ new Date();
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
  const { count: thisMonth, error: monthError } = await supabase.from("vocabulary").select("*", { count: "exact", head: true }).gte("first_seen", thirtyDaysAgo.toISOString());
  if (monthError) throw monthError;
  const { data: allVocab, error: allError } = await supabase.from("vocabulary").select("first_seen").order("first_seen", { ascending: true });
  if (allError) throw allError;
  let averagePerWeek = 0;
  if (allVocab && allVocab.length > 0) {
    const firstDate = new Date(allVocab[0].first_seen);
    const now = /* @__PURE__ */ new Date();
    const weeksSinceStart = Math.max(1, (now - firstDate) / (1e3 * 60 * 60 * 24 * 7));
...
__name(getVocabularyStats, "getVocabularyStats");
...
router2.get("/categories", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const { data: categories, error } = await supabase.from("vocabulary_categories").select("*").order("name", { ascending: true });
    if (error) throw error;
    return c.json({
      success: true,
      data: categories || []
...
router2.post("/categories", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    ...
    const { data: category, error } = await supabase.from("vocabulary_categories").insert({ name: name.trim() }).select().single();
    if (error) {
      if (error.code === "23505") {
...
router2.delete("/categories/:id", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const id = c.req.param("id");
  ...
  try {
    ...
    if (!germanTranslation) {
      try {
        ...
        console.error("Error translating to German:", error2);
      }
    }
    if (germanTranslation) {
      const { data: existing } = await supabase.from("vocabulary").select("*").ilike("word", germanTranslation).single();
      if (existing) {
        return c.json({
          success: false,
...
router2.get("/:id/meaning", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const id = c.req.param("id");
    const { data: word, error } = await supabase.from("vocabulary").select("*").eq("id", id).single();
    if (error) {
      if (error.code === "PGRST116") {
        return c.json({
          success: false,
    ...
    try {
      ...
      await supabase.from("vocabulary").update({ meaning }).eq("id", id);
      return c.json({
        success: true,
        data: {
          word: word.word,
      ...
      console.error("Error fetching meaning:", error2);
      return c.json({
        success: false,
...
router2.put("/:id/review", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const id = c.req.param("id");
    const { error } = await supabase.from("vocabulary").update({ last_reviewed: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id);
    if (error) throw error;
    return c.json({
      success: true,
...
var vocabulary_default = router2;

// routes/phrases.js
var router3 = new Hono2();
var BUILT_IN_PHRASES = [
  ...
  { english: "Can you please explain that?", german: "Kannst du das bitte erkl\xE4ren?", builtin: true },
  ...
  { english: "I would like to suggest that...", german: "Ich m\xF6chte vorschlagen, dass...", builtin: true }
];
router3.get("/", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const { data: customPhrases, error } = await supabase.from("custom_phrases").select("id, english, german, created_at, times_reviewed").order("created_at", { ascending: false });
    if (error) throw error;
...
router3.post("/", async (c) => {
  try {
    ...
    const { data: existing } = await supabase.from("custom_phrases").select("*").or(`english.ilike.${cleanEnglish},german.ilike.${cleanGerman}`).single();
    if (existing) {
      return c.json({
        success: false,
    ...
    const { data: newPhrase, error } = await supabase.from("custom_phrases").insert({
...
router3.put("/:id/review", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const id = c.req.param("id");
    const { data: phrase, error: fetchError } = await supabase.from("custom_phrases").select("times_reviewed").eq("id", id).single();
    if (fetchError) {
      if (fetchError.code === "PGRST116") {
        return c.json({
          success: false,
    ...
    const { error } = await supabase.from("custom_phrases").update({ times_reviewed: (phrase.times_reviewed || 0) + 1 }).eq("id", id);
    if (error) throw error;
    return c.json({
      success: true,
...
router4.get("/stats", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const { count: vocabCount, error: vocabError } = await supabase.from("vocabulary").select("*", { count: "exact", head: true });
    if (vocabError) throw vocabError;
    const { count: entriesCount, error: entriesError } = await supabase.from("journal_entries").select("*", { count: "exact", head: true });
    ...
    const sevenDaysAgo = /* @__PURE__ */ new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    const { count: thisWeekWords, error: weekWordsError } = await supabase.from("vocabulary").select("*", { count: "exact", head: true }).gte("first_seen", sevenDaysAgo.toISOString());
    if (weekWordsError) throw weekWordsError;
    const { count: thisWeekEntries, error: weekEntriesError } = await supabase.from("journal_entries").select("*", { count: "exact", head: true }).gte("created_at", sevenDaysAgo.toISOString());
...
router4.get("/streak", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const { data: entries, error } = await supabase.from("journal_entries").select("created_at").order("created_at", { ascending: false });
    if (error) throw error;
    ...
    const daysDiff = Math.floor((today - lastEntryDate) / (1e3 * 60 * 60 * 24));
    if (daysDiff <= 1) {
      currentStreak = 1;
      tempStreak = 1;
      for (let i = 1; i < dates.length; i++) {
        ...
        const diff = Math.floor((currentDate - prevDate) / (1e3 * 60 * 60 * 24));
        if (diff === 1) {
          currentStreak++;
          tempStreak++;
        } else {
...
router4.get("/history", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const days = parseInt(c.req.query("days")) || 7;
    const startDate = /* @__PURE__ */ new Date();
    startDate.setDate(startDate.getDate() - days);
    const { data: history, error } = await supabase.from("progress_stats").select("date, words_learned, entries_written, minutes_practiced").gte("date", startDate.toISOString().split("T")[0]).order("date", { ascending: true });
    if (error) throw error;
    const result = [];
...
router4.get("/chart-data", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const days = parseInt(c.req.query("days")) || 7;
    const startDate = /* @__PURE__ */ new Date();
...
router4.get("/active-days", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const { data: journalDates, error: journalError } = await supabase.from("journal_entries").select("created_at");
    if (journalError) throw journalError;
    const { data: vocabDates, error: vocabError } = await supabase.from("vocabulary").select("first_seen");
    if (vocabError) throw vocabError;
    const { data: progressDates, error: progressError } = await supabase.from("progress_stats").select("date");
...
router6.get("/", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const { data: settings, error } = await supabase.from("user_settings").select("*").limit(1).single();
...
router6.put("/", async (c) => {
  try {
    ...
    const { data: existing, error: fetchError } = await supabase.from("user_settings").select("*").limit(1).single();
    if (fetchError && fetchError.code !== "PGRST116") {
      throw fetchError;
    }
    if (existing) {
      const updates = {};
      if (daily_goal_minutes !== void 0) updates.daily_goal_minutes = daily_goal_minutes;
      if (daily_sentence_goal !== void 0) updates.daily_sentence_goal = daily_sentence_goal;
      if (theme !== void 0) updates.theme = theme;
      if (Object.keys(updates).length > 0) {
        const { data: updated, error } = await supabase.from("user_settings").update(updates).eq("id", existing.id).select().single();
        if (error) throw error;
      ...
      const { data: newSettings, error } = await supabase.from("user_settings").insert({
...
__name(extractSentencesWithTerm, "extractSentencesWithTerm");
router7.get("/", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const q = c.req.query("q");
    ...
    const { data: vocabResults, error: vocabError } = await supabase.from("vocabulary").select("*").or(`word.ilike.%${searchTerm}%,meaning.ilike.%${searchTerm}%`).order("frequency", { ascending: false }).limit(50);
    if (vocabError) throw vocabError;
    const { data: journalResults, error: journalError } = await supabase.from("journal_entries").select("*").or(`english_text.ilike.%${searchTerm}%,german_text.ilike.%${searchTerm}%`).order("created_at", { ascending: false }).limit(50);
    if (journalError) throw journalError;
...
router8.post("/", async (c) => {
  try {
    ...
    const { data: newNote, error } = await supabase.from("notes").insert({ title: title.trim(), content: content.trim() }).select().single();
    if (error) throw error;
...
router8.put("/:id", async (c) => {
  try {
    ...
    const { data: updatedNote, error } = await supabase.from("notes").update({ title: title.trim(), content: content.trim() }).eq("id", id).select().single();
    if (error) {
      if (error.code === "PGRST116") {
        return c.json({ success: false, error: "Note not found" }, 404);
      }
...
router9.get("/export", async (c) => {
  try {
    const supabase = getSupabaseClient(c.env);
    const { data: journalEntries, error: journalError } = await supabase.from("journal_entries").select("*").order("created_at", { ascending: true });
    if (journalError) throw journalError;
    const { data: vocabulary, error: vocabError } = await supabase.from("vocabulary").select("*").order("first_seen", { ascending: true });
    if (vocabError) throw vocabError;
    const { data: customPhrases, error: phrasesError } = await supabase.from("custom_phrases").select("*").order("created_at", { ascending: true });
    ...
    const { data: progressStats, error: statsError } = await supabase.from("progress_stats").select("*").order("date", { ascending: true });
    ...
    c.header("Content-Disposition", `attachment; filename=deutschtagebuch-backup-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.json`);
...
var data_default = router9;

// index.js
var app = new Hono2();
app.use("*", cors());
app.get("/health", (c) => c.json({ status: "ok", timestamp: (/* @__PURE__ */ new Date()).toISOString() }));
app.route("/journal", journal_default);
app.route("/vocabulary", vocabulary_default);
app.route("/phrases", phrases_default);
app.route("/progress", progress_default);
app.route("/translate", translate_default);
app.route("/settings", settings_default);
app.route("/search", search_default);
app.route("/notes", notes_default);
app.route("/data", data_default);
...
// .wrangler/tmp/bundle-MwpPTk/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-MwpPTk/middleware-loader.entry.ts
